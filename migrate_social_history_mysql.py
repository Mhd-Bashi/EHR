    ...
    with app.app_context():
        try:
            ...
            existing_records = db.session.execute(
                text("SELECT COUNT(*) FROM social_history")
            ).scalar()
                ...
                for record in records:
                    ...
                    if old_smoking_status:
                        if isinstance(old_smoking_status, str):
                            if old_smoking_status.lower() in [
                                "true",
                                "yes",
                                "1",
                                "smoker",
                    ...
                    db.session.execute(
                        text(
                            "UPDATE social_history SET smoking_status = :new_status WHERE id = :record_id"
                        ),
            ...
            try:
                ...
                print(
                    f"⚠️  Note: Column type conversion might need manual intervention: {e}"
                )
