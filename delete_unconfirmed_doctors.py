        ...
        try:
            ...
            for doctor in unconfirmed_doctors:
                ...
                for patient in patients:
                    lab_results += LaboratoryResult.query.filter_by(patient_id=patient.id).count()
                    medical_history += MedicalHistory.query.filter_by(patient_id=patient.id).count()
                ...
                print(f"   Patients: {patient_count}, Appointments: {appointment_count}, Lab Results: {lab_results}, Medical History: {medical_history}")
            ...
            if confirm == 'yes':
                ...
                for doctor in unconfirmed_doctors:
                    print(f"   Processing doctor: {doctor.first_name} {doctor.last_name}")
                    ...
                    for patient in patients:
                        ...
                        lab_count = LaboratoryResult.query.filter_by(patient_id=patient.id).delete()
                        deleted_lab_results += lab_count
                        
                        # Delete medical history for this patient
                        history_count = MedicalHistory.query.filter_by(patient_id=patient.id).delete()
                        deleted_medical_history += history_count
                    
                    # Delete appointments for this doctor
                    appointment_count = Appointment.query.filter_by(doctor_id=doctor.id).delete()
                    deleted_appointments += appointment_count
                    
                    # Delete patients for this doctor
                    patient_count = Patient.query.filter_by(doctor_id=doctor.id).delete()
    ...
    with app.app_context():
        try:
            ...
            for doctor in doctors:
                status = "✅ Confirmed" if doctor.email_confirmed else "❌ Not confirmed"
                print(f"ID: {doctor.id:2d} | {doctor.first_name} {doctor.last_name:15s} | {doctor.email:30s} | {status}")
            ...
            print("-" * 80)
            print(f"📊 Summary: {confirmed_count} confirmed, {unconfirmed_count} unconfirmed")
